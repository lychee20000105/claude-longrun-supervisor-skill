...
The main reason this workflow can still waste supervisor tokens is supervision leakage: the upper-layer supervisor starts reading raw logs, full diffs, process dumps, release pages, and long worker outputs to compensate for missing compact evidence. That defeats the purpose of delegating work to Claude CLI.

Default rule: Claude CLI workers and audit workers spend execution tokens; the upper-layer supervisor spends only decision tokens.

Hard requirements:

- Every worker round must maintain a compact machine-readable summary file named `token-budget-summary.json` at the output root.
- Every worker/audit output must start with a `Supervisor Digest` of at most 12 lines containing: decision needed, files changed, tests run, pass/fail, commit/push/release status, and blockers.
- The upper-layer supervisor must read `token-budget-summary.json`, `longrun-status.json`, and the newest `Supervisor Digest` before reading any raw log, full diff, or long report.
- The upper-layer supervisor must not read raw logs, full command output, full diffs, or full release pages unless the digest says there is a blocker, safety risk, failed validation, missing artifact, or user specifically asks for evidence.
- Background status checks should update files silently. User-facing chat should only report: new user-visible bug, failed/blocked publish, final audit, or an explicit user question.
- When the supervisor needs GitHub publication evidence, prefer one compact command such as `gh release list --limit 5` over opening pages or reading long logs.
- If a worker fails to produce a digest, the next task is to make Claude summarize its own artifacts, not for the supervisor to manually inspect everything.
...
```json
{
  "updatedAt": "ISO-8601",
  "currentRound": 0,
  "state": "running|blocked|draining|completed",
  "decisionNeeded": "none|supervisor|user",
  "latestDigest": "<=1200 chars",
  "changedFiles": [],
  "validation": { "commands": [], "passed": 0, "failed": 0 },
  "publish": { "pushed": false, "releaseUrl": "", "blockedReason": "" },
  "nextCheckAfterSeconds": 1200
}
```
...
- Review compact summaries first (`token-budget-summary.json`, status files, and Supervisor Digests), then only inspect detailed reports when the summary flags risk or missing evidence.
...
- Provide commands, evidence, pass/fail counts, remaining risks, and update `token-budget-summary.json` with a compact digest.
...
The supervisor reads the audit digest first and decides. It reads the full audit report only when the digest flags risk, contradiction, failed validation, or missing evidence.
...
Token budget discipline update.

Design rationale:

- User feedback showed that even when Claude CLI does most execution, Codex can still burn large amounts of token by repeatedly inspecting raw logs, diffs, status dumps, web pages, and long reports.
- The workflow now treats compact summaries as first-class artifacts so Codex remains a decision supervisor instead of becoming a second execution worker.

Behavioral changes:

- Added mandatory `token-budget-summary.json`.
- Added mandatory `Supervisor Digest` at the top of worker and audit outputs.
- Upper-layer supervisors must read compact summaries first and only inspect raw details on blockers, contradictions, failed validation, safety risks, or explicit user request.
- User-facing output cadence is explicitly quiet by default.

Validation:

- Static SKILL.md update; companion scripts updated to generate and reference the summary artifact.
